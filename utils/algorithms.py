...
def compute_entropy_and_mass(mu, stability_threshold=1e-30):
    """
    mu: 2D array of probabilities.
    Compute the entropy H(mu) = - sum( mu * ln(mu) )
    and total mass sum(mu) for a 2D array mu.
    """
    clipped = np.maximum(mu, stability_threshold)
...
def sharpening_find_beta(
    ...
    assert method in ('bisect', 'newton'), "Unknown method."

    def f(beta):
        # b^beta
        safe_b = np.maximum(mu, stability_threshold)
...
def entropic_sharpening(b, H0, stability_threshold=1e-30, verbose=False):
    """
...
def convolutional_wasserstein_barycenter(
    ...
    Entropy_sharpening : bool, optional. Choose whether to apply entropy sharpening.
    H0 : float, optional. Defaults to the maximum entropy of the input distributions.
    ...
    for iteration in range(max_iterations):
        ...
        if entropy_sharpening:
            ...
            barycenter = entropic_sharpening(barycenter, H0, verbose=verbose)
